"""
Markup fixer + audit for sch.txt (SCH).
Counterpart of PWG#175, PWK#113, and siblings.

SCH-specific notes: 0 whitespace hits. 1 non-standard <ab n> (DHATATUPATHA).

Inputs: ../../../csl-orig/v02/sch/sch.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "sch" / "sch.txt", HERE / "sch.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "sch_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(ls|sup|lang|ab)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for sch.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"SCH markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against sch.txt.\n")
        f.write(f"Paired tags: <ls>, <sup>, <lang>, <ab>\n")
        f.write(f"Notes: 0 whitespace hits. 1 non-standard <ab n> (DHATATUPATHA).\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <ls>, <sup>, <lang>, <ab>\n\n")
